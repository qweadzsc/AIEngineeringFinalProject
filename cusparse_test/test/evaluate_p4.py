...
def main(args):
    ...
    src_path = "/share/public/zhouyongkang/projects/sc/src"
    cuda_module = load(name="mlp_kernel",
                       sources=[f"{src_path}/api.cu",
                                f"{src_path}/impl.cu",
                                f"{src_path}/dsmm/dense_sddmm.cu",
                                f"{src_path}/dsmm/dense_spmm.cu"],
    ...
    # for i in range(32):
        # if i not in [2, 3, 4, 5, 6, 7]:
            # model.model.layers[i].mlp = MLPCUDA(model.model.layers[i].mlp, cuda_module)
