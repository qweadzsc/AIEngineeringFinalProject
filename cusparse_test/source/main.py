...
def profile_batched_gen(model, tokenizer, ds, batch_size, device, n_run=100):
    ...
    for i in tqdm(range(n_run)):
        ...
        input_ids = tokenizer(input_text, return_tensors='pt', padding='longest').to(device)
        start = time.time()
        output = model.generate(input_ids.input_ids, attention_mask=input_ids.attention_mask, max_new_tokens=1, do_sample=False)
        total_time += time.time() - start
        print(tokenizer.decode(output[0]))
        exit(-1)
    # with open("/share/xujiaming/train_machine/yongkang/cusparse_test/result/batch_profile/original_time_wo_pred.txt", 'w') as f:
    #     print(f"totol time used in gen for batch size {batch_size} is {total_time:.6f}s", file=f)
...
if __name__ == "__main__":
    ...
    cutlass_path = "/share/xujiaming/train_machine/yongkang/cusparse_test/deps/cutlass"
    cuda_module = load(name="cutlass_mlp", sources=["api.cu", "impl.cu"],
                       extra_include_paths=[f'{cutlass_path}/include'], verbose=True)
    ...
    profile_batched_gen(model, tokenizer, pds, 8, device, 10)
