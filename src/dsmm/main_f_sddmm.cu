...
sg(const void * x, const void * up, const void * gate, const void * mask,
   void * r, void * val, long long * col, int m, int n, int p, int k) {
    ...
    using G2RCopyMask = typename Config::G2RCopyMask;
...
#pragma unroll
    ...
    p_val = __shfl_xor_sync(0xffffffff, max_val, 8U, 32);
    col_off[1] = __shfl_xor_sync(0xffffffff, col_off[0], 8U, 32);
    if (p_val > max_val) {
        col_off[0] = col_off[1];
    }
    p_val = __shfl_xor_sync(0xffffffff, max_val, 16U, 32);
    col_off[1] = __shfl_xor_sync(0xffffffff, col_off[0], 16U, 32);
    if (p_val > max_val) {
        col_off[0] = col_off[1];
    }
    ...
    auto tSXsSX = s2r_thr_copy_spx.partition_S(local_tile(
        sX, make_tile(_1{}, _64{}), make_coord(idx%8+iw*8, 0)
    ));
    ...
    auto tSUsSU = s2r_thr_copy_spup.partition_S(local_tile(
        sSU, make_tile(_1{}, _64{}), make_coord(col_off[(idx/8)%2], 0)
    ));
    ...
    auto tSGsSG = s2r_thr_copy_spgate.partition_S(local_tile(
        sSG, make_tile(_1{}, _64{}), make_coord(col_off[(idx/8)%2], 0)
    ));
...
#pragma unroll
        for (int sp = 0; sp < sp_cal_n; ++sp) {
            ...
            rhs = tCrSR(sp, 1);
            lhs = (lhs>0&&rhs>0)?(lhs*rhs):0;
            ((T *)val)[nz_st+(idx/4)*2+sp] = static_cast<T>(lhs);
        }
    }
    if (il < 8) {
        col[nz_st+il*2+iw*16] = col_off[0];
        col[nz_st+il*2+iw*16+1] = col_off[1];
    }
