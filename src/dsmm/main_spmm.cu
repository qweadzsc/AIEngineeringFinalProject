...
sg(const void * x, const void * down, void * r,
   const void * val, const long long * row, const long long * col,
   int m, int n, int p, int k) {
    ...
    int spiy = iy / 4;
    int nz_st = row[spix];
    int nnz = row[spix+1] - nz_st;
    int debug = (ix + iy + iz) * 128 + idx;
...
int main() {
    ...
    CUDA_CHECK(cudaMalloc(&d_r1,   sizeof(T) * m * n * 5));
    ...
    auto tr1 = make_tensor(h_r1,   make_shape(m, n, 4), make_stride(n, 1, m * n));
    ...
    for (int i = 0; i < n; ++i) {
        rand_col_offset(h_col_off+i*nnz_pc, k-p, nnz_pc);
    }
    ...
    CUDA_CHECK(cudaMemset(d_r1, 0, sizeof(T)*m*n*5));
    ...
    dim3 block(128);
    dim3 grid((n + kTileN - 1) / kTileN, (m + kTileM - 1) / kTileM, kBlock);
    ...
    for (int i = 0; i < run_num; ++i) {
        ...
        sg<decltype(cfg)><<<grid, block, shm_size, stream>>>(d_x, d_down, d_r1, d_val, d_row_ind,
                                                             d_col_off, m, n, p, k);
        dim3 block_add(256);
        dim3 grid_add((m*n+255)/256);
        gpu_add<T, kBlock><<<block_add, grid_add, 0, stream>>>(d_r1, m*n);
