...
template <typename T_, int kTileM_=64, int kTileN_=16, int kTileK_=64, int kTileS_=4,
...
// 32x8x64, 80, 32
template <typename T_, int kStage_>
struct Config<T_, 32, 8, 64, 80, 32, kStage_> {
    using T = T_;
    static constexpr int kTileM = 32;
    static constexpr int kTileN = 8;
    static constexpr int kTileK = 64;
    static constexpr int kTileS = 80;
    static constexpr int maxnnz = 32;
    static constexpr int kStage = kStage_;

    // g2s config
    using SwizzleSize = Swizzle<3, 3, 3>;
    using SmemLayoutAtom = decltype(composition(
        SwizzleSize{},
    ...
    using SmemLayoutAtomT = decltype(composition(
        SwizzleSize{},
        make_layout(make_shape(Int<kTileK>{}, Int<512/kTileK>{}),
                    make_stride(Int<512/kTileK>{}, Int<1>{}))
    ));
    using SmemLayoutX = decltype(tile_to_shape(
        SmemLayoutAtomT{}, make_shape(Int<kTileK>{}, Int<kTileN>{}, Int<kStage>{})
    ));
    using SmemLayoutDown = decltype(tile_to_shape(
        SmemLayoutAtom{}, make_shape(Int<kTileM>{}, Int<kTileK>{}, Int<kStage>{})
    ));
    ...
    using SmemLayoutVal = decltype(make_layout(make_shape(_64{}, Int<kStage>{}),
                                               make_stride(_1{}, _64{})));
    using SmemLayoutCol = decltype(tile_to_shape(
        composition(Swizzle<2, 3, 1>{}, make_layout(make_shape(_64{}, _1{}),
                                                    make_stride(_1{}, _64{}))),
        make_shape(_64{}, Int<kStage>{})
    ));
