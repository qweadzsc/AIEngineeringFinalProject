...
class MixedSDDMMMetadataTest(unittest.TestCase):
    def test_build_metadata_splits_dense_and_sparse_regions(self) -> None:
        ...
        self.assertTrue(torch.equal(metadata.mask_c_dense, torch.tensor([1, 0], dtype=torch.long)))
        self.assertTrue(torch.equal(metadata.mask_c_sparse, torch.tensor([2, 3, 4], dtype=torch.long)))
        self.assertTrue(
            torch.equal(
    ...
    def test_sparse_width_zero_is_legal(self) -> None:
        ...
        self.assertTrue(torch.equal(metadata.mask_c_dense, torch.tensor([0, 2], dtype=torch.long)))
